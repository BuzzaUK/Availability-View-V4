...
async function fixShift74Timestamps() {
  try {
    console.log('🔧 Fixing Shift 74 Timestamps');
    console.log('=' .repeat(60));
    
    // Get Shift 74 details
    const shift74 = await Shift.findByPk(74);
    if (!shift74) {
      console.log('❌ Shift 74 not found!');
      return;
    }
    
    console.log(`📊 Shift 74: ${shift74.shift_name}`);
    console.log(`   Start: ${shift74.start_time}`);
    console.log(`   End: ${shift74.end_time}`);
    
    // Parse shift times
    const shiftStart = moment(shift74.start_time);
    const shiftEnd = moment(shift74.end_time);
    ...
    // Get all events for Shift 74
    const events = await Event.findAll({
      where: { shift_id: 74 },
      order: [['id', 'ASC']]
    });
    
    console.log(`\n📋 Found ${events.length} events for Shift 74`);
    ...
    console.log(`\n🎉 Shift 74 timestamp fix completed!`);
