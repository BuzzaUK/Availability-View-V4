...
(async () => {
  try {
    console.log('🔄 Setting up 13:08 GMT shift trigger...');
    console.log('Current time:', new Date().toLocaleString());
    
    // Add 13:08 as a shift time
    const settings = await databaseService.getNotificationSettings();
    console.log('Current shift times:', settings.shiftSettings.shiftTimes);
    
    // Add 1308 (13:08) if not already present
    if (!settings.shiftSettings.shiftTimes.includes('1308')) {
      settings.shiftSettings.shiftTimes.push('1308');
      await databaseService.updateNotificationSettings(settings);
      console.log('✅ Added 13:08 shift time');
    } else {
      console.log('⚠️ 13:08 shift time already exists');
    }
    ...
    console.log('⏰ 13:08 shift will trigger automatically when the time comes');
